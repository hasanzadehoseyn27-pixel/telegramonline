...
# فرمت هر آیتم: (کلید, [[معادل‌های گروه۱], ...], [کلمات_ممنوع_اختیاری])
_WHITELIST: list[tuple[str, list[list[str]], list[str]]] = [
    ...
    ("corolla_cross", [["کرولا"], ["کراس"]], ["الیت", "چرم"]),
...
def match_zero_whitelist(vehicle_name: str | None, trim: str | None, raw_text: str | None = None) -> str | None:
    ...
    ⚠️ عمداً raw_text رو چک نمی‌کنیم — کلمات لیست سفید (مثل «کراس»،
    «سانروف»، «ایگل») ممکنه هرجای متن خام، بی‌ربط به اسم اصلی خودرو،
    پیدا بشن. فقط دو فیلدی که خودِ parser به‌عنوان «اسم خودرو» تشخیص
    داده (vehicle_name/trim) رو چک می‌کنیم، و مطابقت هم کلمه‌به‌کلمه‌ست
    (word boundary)، نه فقط زیررشته‌ی آزاد.
    """
