...
DETAILED_VEHICLE_PATTERNS: list[tuple[str, str, str]] = [
    ...
    ("shahin_auto_plus", "شاهین اتومات پلاس", r"شاهین.*(?:اتو|اتومات|اتوماتیک).*(?:پلاس)"),
    ...
    ("haima_s5", "هایما S5", r"هایما\s*[\(\s]*(?:s\s*5|s5|اس\s*5)"),
    ("haima_s7", "هایما S7", r"هایما\s*[\(\s]*(?:s\s*7|s7|اس\s*7)"),
    ...
    ("honda_hrv", "هوندا HR-V", r"هوندا\s*(?:hr\s*-?\s*v|hrv)"),
    ...
    ("rav4_china_dual", "تویوتا راو۴ چین دو دیفرانسیل", r"(?:تویوتا\s*)?(?:رافور|راوفور|راو\s*4|rav\s*4).*(?:چین).*(?:2\s*دف|دو\s*دف|دو\s*دیف|دو\s*دیفرانسیل)"),
    ("rav4_single_diff", "تویوتا راو۴ تک دیفرانسیل", r"(?:تویوتا\s*)?(?:رافور|راوفور|راو\s*4|rav\s*4).*(?:تک\s*دف|تک\s*دیف|تک\s*دیفرانسیل)"),
    ("nissan_altima", "نیسان آلتیما", r"نیسان\s*(?:آ|ا)لتیما|altima"),
...
IMPORTED_YEAR_HINT_RE = re.compile(
    ...
    r"byd|بی\s*وای\s*دی|toyota|تویوتا|لوین|تیراک|رافور|راوفور|راو|rav|nissan|نیسان|قشقایی|benz|بنز|elantra|النترا|فراتلندر|فرانتلندر",
    re.IGNORECASE,
)
